...
# StaphScope SCCmec Summary Generator
# Usage: ./generate_staphscope_summary.sh

echo "=================================================="
echo "        STAPHSCOPE Summary Report Generator"
echo "=================================================="

# ASCII Art for StaphScope (new solid block style)
print_ascii_art() {
    cat << 'EOF'
███████╗████████╗ █████╗ ██████╗ ██╗  ██╗███████╗ ██████╗ ██████╗ ██████╗ ███████╗
██╔════╝╚══██╔══╝██╔══██╗██╔══██╗██║  ██║██╔════╝██╔════╝██╔═══██╗██╔══██╗██╔════╝
...
# Initialize TSV file with simplified header (only 3 columns)
echo -e "Sample_Name\tSCCmec_Type\tMRSA_Status" > "$TSV_SUMMARY"
...
for dir in $s_dirs; do
    ...
    if [ ! -f "$results_file" ]; then
        echo "  ⚠️  No results file found, skipping..."
        continue
    fi
    ...
    while IFS= read -r line; do
        ...
        # Extract SCCmec type
            ...
            sccmec_type=$(echo "$line_clean" | sed 's/Prediction based on genes: //')
        fi
    done < "$results_file"
    
    # Add to TSV (only 3 columns)
    echo -e "${sample_name}\t${sccmec_type}\t${mrsa_status}" >> "$TSV_SUMMARY"
    
    # Store for HTML
    results_data+=("$sample_name" "$sccmec_type" "$mrsa_status")
    
    echo "  ✅ $mrsa_status | $sccmec_type"
...
<head>
    ...
    <title>STAPHSCOPE - SCCmec Summary Report</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <div class="summary-section">
            ...
            <div class="metrics-grid">
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value">$(grep -c "MRSA" "$TSV_SUMMARY")</div>
                </div>
                <div class="metric-card">
                    <div class="metric-label">MSSA Isolates</div>
                    <div class="metric-value">$(grep -c "MSSA" "$TSV_SUMMARY")</div>
                </div>
                <div class="metric-card">
                    <div class="metric-label">SCCmec Types Found</div>
                    <div class="metric-value">$(grep -v "Not Detected" "$TSV_SUMMARY" | grep -v "Sample_Name" | wc -l)</div>
                </div>
            </div>
        </div>
...
EOF
    ...
    # Add table rows from results data (only 3 columns)
    for ((i=0; i<${#results_data[@]}; i+=3)); do
    ...
    cat << EOF
        ...
        <div class="footer">
            <p><strong>STAPHSCOPE</strong> - SCCmec Cassette Analysis Summary</p>
...
echo "=================================================="
echo "📁 TSV Summary: $TSV_SUMMARY"
echo "🌐 HTML Summary: $HTML_SUMMARY"
echo "📊 Total samples processed: $total_processed"
echo
echo "Summary Statistics:"
echo "-------------------"
echo "MRSA isolates: $(grep -c "MRSA" "$TSV_SUMMARY")"
echo "MSSA isolates: $(grep -c "MSSA" "$TSV_SUMMARY")"
echo "SCCmec types detected: $(grep -v "Not Detected" "$TSV_SUMMARY" | grep -v "Sample_Name" | wc -l)"
echo
print_ascii_art
