#!/usr/bin/env python3
"""
StaphScope Main Orchestrator - FINAL VERSION WITH FASTA FORMAT SUPPORT
...
class StaphScopeOrchestrator:
    """Final StaphScope orchestrator - supports ALL FASTA formats"""
    ...
    def run_mlst_analysis(self, fasta_files: List[Path], output_dir: Path, threads: int) -> bool:
        """Run MLST analysis - SUPPORTS ALL FASTA FORMATS"""
        try:
            ...
            # Build command - uses correct file pattern
            cmd = [
            ...
            if result.returncode == 0:
                ...
                for fasta_file in fasta_files:
                    temp_file = mlst_module_path / fasta_file.name
                    if temp_file.exists():
                        temp_file.unlink()
    ...
    def run_spa_typing(self, fasta_files: List[Path], output_dir: Path, threads: int) -> bool:
        """Run spa typing - SUPPORTS ALL FASTA FORMATS"""
        try:
            ...
            # Build command - spa typing uses "." for current directory
            cmd = [
            ...
            if result.returncode == 0:
                ...
                # Clean up copied input files
                for fasta_file in fasta_files:
                    temp_file = spa_module_path / fasta_file.name
                    if temp_file.exists():
                        temp_file.unlink()
    ...
    def run_sccmec_analysis(self, fasta_files: List[Path], output_dir: Path, threads: int) -> bool:
        """Run SCCmec analysis - SUPPORTS ALL FASTA FORMATS"""
        try:
            ...
            # Get correct file pattern based on actual files
            file_pattern = self.get_file_pattern(fasta_files)
            # Remove quotes for bash command
            ...
            if result.returncode == 0:
                ...
                # Clean up copied input files and generated directories
                for fasta_file in fasta_files:
                    temp_file = sccmec_module_path / fasta_file.name
                    if temp_file.exists():
                        temp_file.unlink()
                
                # Clean up s_* directories from module directory
                for s_dir in sccmec_module_path.glob("s_*"):
                    if s_dir.is_dir():
                        shutil.rmtree(s_dir)
                
                # Clean up summary files from module directory
                for summary_file in summary_files:
                    temp_summary = sccmec_module_path / summary_file
    ...
    def run_amrfinder_analysis(self, fasta_files: List[Path], output_dir: Path, threads: int) -> bool:
        """Run AMRFinderPlus analysis - SUPPORTS ALL FASTA FORMATS"""
        try:
            ...
            if result.returncode == 0:
                ...
                # Clean up copied input files
                for fasta_file in fasta_files:
                    temp_file = amr_module_path / fasta_file.name
                    if temp_file.exists():
                        temp_file.unlink()
    ...
    def run_abricate_analysis(self, fasta_files: List[Path], output_dir: Path, threads: int) -> bool:
        """Run Abricate analysis - SUPPORTS ALL FASTA FORMATS"""
        try:
            ...
            if result.returncode == 0:
                ...
                # Clean up copied input files
                for fasta_file in fasta_files:
                    temp_file = abricate_module_path / fasta_file.name
                    if temp_file.exists():
                        temp_file.unlink()
