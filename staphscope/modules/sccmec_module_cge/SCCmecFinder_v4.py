...
Updated: 2026-09-16 (Fixed mecC detection and MyDbFinder header-skip bug; LGA251/SCCmec XI now resolves correctly)
...
Usage:
    python3 SCCmecFinder_v4.py -iDb sample.fna -iKm sample.fna \
        -o sccmec_detailed_results.txt -d output_dir \
        -db_dir database -sc_dir script_dir -db_choice reference
"""
...
def get_top_kmer_predictions(kmer_results: List[Dict], has_mec_gene: bool) -> Dict[str, Any]:
    """
    Get top k-mer based predictions when gene-based typing is inconclusive.
    Returns the top 2 hits ranked by template coverage.
    """
    if not kmer_results or not has_mec_gene:
        return {
...
def generate_enhanced_json_report(sample_name: str, mrsa_gene: str, sccmec_types: List[str],
    ...
    """
    Generate the enhanced JSON report summarising MRSA status, SCCmec typing,
    k-mer prediction, gene detection, and quality metrics.
    """
    primary_type = sccmec_types[0] if sccmec_types else "NA"

    mrsa_evidence = []
    if mrsa_gene:
        mrsa_evidence.append(f"{mrsa_gene}_detected")
    ...
    enhanced_report = {
        ...
        "gene_detection": {
            ...
            "regulator_genes": [
                {"gene": gene, "detected": True}
                for gene in sorted(
                    g for g in total_genes
                    if any(x in g for x in ['mecR', 'mecI', 'IS1272', 'dmec'])
                )
            ]
        },
        "quality_metrics": {
...
def main():
    ...
    if os.path.getsize(db_finder_file) == 0:
        ...
        with open(db_finder_file, 'r') as f:
            for line in f:
                ...
                gene = parts[0].split(':')[0].split('|')[0].strip()
    ...
    # Get k-mer based prediction for isolates with an MRSA determinant
    has_mec_gene = bool(mrsa_gene)
    kmer_prediction = get_top_kmer_predictions(detailed_kmer_results, has_mec_gene)
